...
including basic functionality, timing verification, and error handling scenarios.
...
class MockBot(Bot):
    """A mock bot class that simulates Bot behavior for testing parallel dispatch functionality.

    Use when you need to test parallel bot operations without making actual API calls.
    Simulates basic bot behavior by returning predictable responses based on bot name.
    ...
    Attributes:
        ...
        conversation (ConversationNode): Tracks conversation history in a tree structure
    """
    ...
    def respond(self, message: str) -> str:
        ...
        Returns:
            str: A mock response in the format "Mock response from {bot_name}: {message}"
        """
...
class TestParDispatch(unittest.TestCase):
    ...
    Tests the parallel execution of multiple bots using par_dispatch, including:
    - Basic functionality with simple prompts
    - True parallel execution verification
    - Error handling across multiple bots

    Attributes:
        bots (List[MockBot]): List of mock bot instances used for testing parallel operations
    """

    def setUp(self):
        """Initialize test environment with three mock bots.

        Creates three MockBot instances with names 'test_bot_0' through 'test_bot_2'.
        """
    ...
    def test_basic_dispatch(self):
        ...
        - Each result is properly structured (tuple of str and ConversationNode)
    ...
    def test_parallel_execution(self):
        ...
        Uses a deliberately slow prompt function to verify that total execution time
        is less than would be required for sequential execution.
