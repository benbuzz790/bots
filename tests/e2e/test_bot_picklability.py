...
def test_pickle_bot_attributes():
    ...
    for attr_name, attr_value in bot.__dict__.items():
        ...
        try:
            ...
            results[attr_name] = "âÅ“… PICKLABLE"

        except Exception as e:

            results[attr_name] = f"âÂÅ’ NOT PICKLABLE: {type(e).__name__}: {str(e)[:100]}"
...
def test_tool_handler_components():
    ...
    try:
        ...
        print(f"  âÂÅ’ {type(e).__name__}: {e}")
    ...
    for func_name, func in list(handler.function_map.items())[:3]:  # Test first 3
        ...
        try:
            ...
            print(f"  âÅ“… {func_name}: PICKLABLE")

        except Exception as e:

            print(f"  âÂÅ’ {func_name}: {type(e).__name__}")
    ...
    for module_name, module_ctx in list(handler.modules.items())[:2]:  # Test first 2
        ...
        try:
            ...
            print(f"âÂÅ’ {type(e).__name__}: {str(e)[:60]}")
    ...
    for module_name, module_ctx in list(handler.modules.items())[:1]:  # Just test first one
        ...
        for key, value in list(module_ctx.namespace.__dict__.items())[:10]:  # First 10
            ...
            try:
                ...
                print(f"  âÅ“… {key}: {type(value).__name__}")

            except Exception as e:

                print(f"  âÂÅ’ {key}: {type(value).__name__} - {type(e).__name__}")
...
def test_deepcopy_bot():
    ...
    try:
        ...
        print("âÅ“… Bot can be deepcopied!")

        return True

    except Exception as e:

        print("âÂÅ’ Bot cannot be deepcopied:")
...
def test_deepcopy_functionality():
    ...
    if bot1.conversation != bot2.conversation:
        ...
        print("   âÅ“… Bots have independent conversations")

    else:

        print("   âÂÅ’ Bots share conversation (not independent!)")
    ...
    if len(bot2.tool_handler.tools) == len(bot1.tool_handler.tools):
        ...
        print(f"   âÅ“… Copied bot has same number of tools ({len(bot2.tool_handler.tools)})")

    else:

        print(f"   âÂÅ’ Tool count mismatch: {len(bot1.tool_handler.tools)} vs {len(bot2.tool_handler.tools)}")

    # Check function_map

    if len(bot2.tool_handler.function_map) == len(bot1.tool_handler.function_map):

        print(f"   âÅ“… Copied bot has same number of functions ({len(bot2.tool_handler.function_map)})")

    else:

        print("   âÂÅ’ Function count mismatch")

    print("\nâÅ“… DEEPCOPY FUNCTIONALITY TEST COMPLETE")
