...
class TestBranchSelfParallelTools:
    ...
    def test_branch_self_handles_parallel_tool_calls(self):
        ...
        with patch("copy.deepcopy") as mock_deepcopy:
            ...
            # Call branch_self with _bot parameter
            branch_self(self_prompts="['test 1', 'test 2']", allow_work="False", _bot=bot)
    ...
    def test_branch_self_without_parallel_tools(self):
        """Test that branch_self works correctly when there are no parallel tool calls."""
        bot = Mock(spec=Bot)
        bot.autosave = False

        # Create a mock conversation node WITHOUT tool calls
        original_node = Mock(spec=ConversationNode)
        original_node.content = "Regular message"
        ...
        with patch("copy.deepcopy") as mock_deepcopy:
            ...
            # Call branch_self with _bot parameter
            result = branch_self(self_prompts="['test 1', 'test 2']", allow_work="False", _bot=bot)

        # Should complete successfully even without tool calls
