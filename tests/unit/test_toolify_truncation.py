...
class TestToolifyTruncation(unittest.TestCase):
    ...
    def test_normal_large_output_not_truncated(self):
        """Test that outputs below threshold (20k chars) are not truncated."""
        # 10k characters - well below the 20k threshold
        large_text = "A" * 10000
    ...
    def test_very_large_output_not_truncated(self):
        """Test that outputs at threshold (20k chars) are not truncated."""
        # 20k characters - exactly at the threshold
        very_large_text = "B" * 20000
    ...
    def test_pathological_output_truncated(self):
        """Test that pathological outputs (> 20k chars) are truncated."""
        # Create output longer than threshold (20k chars)
        pathological_text = "C" * 30000
        ...
        # Should preserve start and end (8000 chars each)
    ...
    def test_truncation_preserves_start_and_end(self):
        """Test that truncation preserves 8000 chars from start and end."""
        # Create text with identifiable start and end
        pathological_text = "START" + ("X" * 30000) + "END"
        result = _convert_tool_output(pathological_text)
    ...
    def test_output_at_exact_threshold_not_truncated(self):
        """Test that output exactly at threshold (20k) is not truncated."""
        text = "D" * 20000
    ...
    def test_output_just_over_threshold_is_truncated(self):
        """Test that output just over threshold (20,001) is truncated."""
        text = "E" * 20001
        result = _convert_tool_output(text)
        # Should be truncated
        self.assertIn("truncated", result)
    ...
    def test_json_output_truncation(self):
        ...
        # Create massive dict that will serialize to > 20k chars
        large_dict = {f"key_{i}": f"value_{i}" * 100 for i in range(200)}
...
class TestToolifyTruncationIntegration(unittest.TestCase):
    ...
    def test_massive_file_reading_simulation(self):
        ...
        # Simulate massive file content (> 20k chars)
