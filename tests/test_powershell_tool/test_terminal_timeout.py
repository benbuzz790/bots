...
python create_sample.py"""
        ...
        with session:
            ...
            try:
                ...
                elapsed = time.time() - start_time
                print(f"✅ Problematic code completed in {elapsed:.2f}s")
                print(f"Result preview: {repr(result[:200])}...")
            except TimeoutError as e:
                elapsed = time.time() - start_time
                print(f"❌ Problematic code timed out after {elapsed:.2f}s")
                # Don't fail - we expect this might timeout
    ...
    def test_delimiter_detection(self):
        ...
        with session:
            ...
            try:
                ...
                delimiter_found = any("COMMAND_" in str(item) and "_COMPLETE" in str(item) 
                                    for item in received_items if item)
                if delimiter_found:
...
'@"""
            ...
            try:
                ...
                result = session.execute(wrapped.replace(delimiter, "TEST_DELIMITER"), timeout=5)
                print(f"✅ Wrapped code executed successfully")
                print(f"Result: {repr(result)}")
            except Exception as e:
...
'@ | Out-File test_alt.txt"""
    ...
    def test_process_communication_health(self):
        ...
        with session:
            ...
            for i in range(5):
                try:
                    result = session.execute(f"Write-Output 'Test {i}'", timeout=2)
