...
def bot_with_branches(bot):
    """Create a bot with a conversation tree that has multiple branches and leaves."""
...
def test_broadcast_fp_chain(bot_with_branches):
    ...
    assert len(responses) == 3
    assert len(nodes) == 3
    # All responses should be strings (not None) - these are the final responses from each chain
    successful_responses = [r for r in responses if r is not None]
...
def test_broadcast_fp_with_skip_labels(bot_with_branches):
    ...
    responses, nodes = fp.broadcast_fp(bot_with_branches, fp.single_prompt, skip=["draft", "test"], prompt="Process this")
...
def test_broadcast_fp_no_skip_parameter(bot_with_branches):
    """Test broadcast_fp without skip parameter (should default to empty list)."""
...
def test_broadcast_fp_tree_of_thought(bot_with_branches):
    ...
    responses, nodes = fp.broadcast_fp(bot_with_branches, fp.tree_of_thought, prompts=["Consider aspect A", "Consider aspect B"], recombinator_function=simple_recombinator)
    # Should have responses from all leaves
    assert len(responses) == 3
    assert len(nodes) == 3
...
def test_broadcast_fp_with_kwargs(bot_with_branches):
    ...
    responses, nodes = fp.broadcast_fp(bot_with_branches, fp.prompt_while, first_prompt="Start task", continue_prompt="Continue", stop_condition=fp.conditions.tool_not_used)
    # Should have responses from all leaves
    assert len(responses) == 3
    assert len(nodes) == 3