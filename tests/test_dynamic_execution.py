...
class TestDynamicFunctionExecution(unittest.TestCase):
    ...
    def test_view_dir_dynamic_execution_with_imports(self):
        """Test that view_dir works when dynamically executed (reproduces the original bug)."""
        ...
        self.assertIn('handle_errors', context, "handle_errors decorator should be in context")
    ...
    def test_old_context_method_fails(self):
        """Test that the old context method would fail (demonstrates the bug)."""
        func = view_dir
        code = func.__code__
        names = code.co_names
        # Create context like the OLD bot system did (without the fix)
        old_context = {name: func.__globals__[name] for name in names if name in func.__globals__}
