...
class DummyToolHandler(ToolHandler):
    ...
    def generate_tool_schema(self, func):
        ...
        return {'name': func.__name__, 'description': func.__doc__ or '', 'parameters': {param: {'type': 'any'} for param in func.__code__.co_varnames[:func.__code__.co_argcount]}}
...
class TestToolHandlerPersistence(unittest.TestCase):
    ...
    def compare_tools(self, original_tools: List[Dict], loaded_tools: List[Dict], context: str):
        ...
        self.assertEqual(len(original_tools), len(loaded_tools), f'{context}: Different number of tools after loading')
        ...
        for orig, loaded in zip(original_tools, loaded_tools):
            ...
            self.assertEqual(orig['description'], loaded['description'], f"{context}: Tool descriptions don't match")
            self.assertEqual(orig['parameters'], loaded['parameters'], f"{context}: Tool parameters don't match")
    ...
    def test_module_persistence(self):
        ...
        self.assertEqual(sorted(new_handler.function_map.keys()), ['module_tool_1', 'module_tool_2'], 'Missing expected functions')
        test_value = 5
...
class TestDecoratorHandling(unittest.TestCase):
    ...
    def test_single_decorator_handling(self):
        """Test that a single decorator is properly included in execution context"""
        # Test with the actual handle_errors decorator which is available globally
        ...
        self.assertEqual(result, "10")  # Should return the result without decoration for successful calls
    ...
    def test_multiple_decorators_handling(self):
        ...
        from bots.dev.decorators import handle_errors, debug_on_error
        ...
        self.assertEqual(result, "42")  # Should return the result for successful calls

    def test_handle_errors_decorator(self):
        """Test the specific handle_errors decorator that caused the original issue"""
        ...
        self.assertIn("Tool Failed", result)  # handle_errors returns "Tool Failed: ..." format
