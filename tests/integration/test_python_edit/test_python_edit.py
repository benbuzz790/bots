...
def test_replace_function_with_multiple_functions_and_imports(tmp_path):
    """Test replacing a function with multiple functions including imports (CodeRabbit suggestion)"""
    content = """
def func_one():
    return "one"
...
"""
    ...
    # Replace func_one with import + three functions
    result = python_edit(
        f"{test_file}::func_one",
        """
from typing import Optional

def func_one(x: Optional[int] = None):
    return "one_modified"
...
""",
    ...
    # Find line indices
    lines = content.split("\n")
    import_line = next(i for i, line in enumerate(lines) if "from typing import Optional" in line)
    func_one_line = next(i for i, line in enumerate(lines) if "def func_one" in line)
    helper_a_line = next(i for i, line in enumerate(lines) if "def helper_a" in line)
    helper_b_line = next(i for i, line in enumerate(lines) if "def helper_b" in line)
    func_two_line = next(i for i, line in enumerate(lines) if "def func_two" in line)

    # Assert import comes before func_one
    assert import_line < func_one_line, "Import should come before func_one"

    # Assert correct ordering: func_one → helper_a → helper_b → func_two
    assert func_one_line < helper_a_line, "func_one should come before helper_a"
    assert helper_a_line < helper_b_line, "helper_a should come before helper_b"
    assert helper_b_line < func_two_line, "helper_b should come before func_two"

    # Verify all content is present
    assert "from typing import Optional" in content
    assert "def func_one(x: Optional[int] = None):" in content
    assert "def helper_a():" in content
    assert "def helper_b():" in content
    assert "def func_two():" in content
