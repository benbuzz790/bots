...
class TestAddTools2(unittest.TestCase):
    ...
    def setUp(self):
        self.bot = AnthropicBot(model_engine=Engines.CLAUDE35_SONNET_20241022, max_tokens=1000, temperature=0, name='TestBot', autosave=False)
    ...
    def test_save_load_dynamic_function(self):
        ...
        save_path = os.path.join('test_outputs', f'dynamic_func_{self.bot.name}')
        ...
        self.assertEqual(self.bot.tool_handler.function_map['dynamic_func'].__doc__, loaded_bot.tool_handler.function_map['dynamic_func'].__doc__)
    ...
    def test_save_load_mixed_sources(self):
        ...
        save_path = os.path.join('test_outputs', f'mixed_sources_{self.bot.name}')
        os.makedirs('test_outputs', exist_ok=True)
        save_path = self.bot.save(save_path)
        loaded_bot = bots.load(save_path)
        self.assertEqual(len(self.bot.tool_handler.tools), len(loaded_bot.tool_handler.tools))
