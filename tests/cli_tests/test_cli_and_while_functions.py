...
class MockBot:
    ...
    def respond(self, prompt):
        if self.response_index < len(self.responses):
            ...
            response = self.responses[-1] if self.responses else "Default response"
...
def test_par_branch_while_with_prompt_array():
    ...
    with patch('os.remove'), patch.object(MockBot, 'save'), patch.object(MockBot, 'load', return_value=MockBot(["Par response"])):
...
def test_chain_while_with_callback():
    ...
    assert callback_responses == responses, "Callback should receive all responses"
...
def test_while_functions_with_single_prompt():
    """Test that *_while functions work with single prompt (backward compatibility)."""
    def stop_immediately(bot):
        return True
