...
class TestDescopedFunctions(unittest.TestCase):
    ...
    def test_prompt_for_excluded_from_discovery(self):
        ...
        self.assertNotIn('prompt_for', discovered_functions, "prompt_for should be excluded from CLI interface")

    def test_par_dispatch_excluded_from_discovery(self):
        """Test that par_dispatch is excluded from function discovery."""
        discovered_functions = self.handler.fp_functions
        self.assertNotIn('par_dispatch', discovered_functions, "par_dispatch should be excluded from CLI interface")
    ...
    def test_dynamic_prompt_parameter_handler_shows_not_supported(self):
        """Test that dynamic_prompt parameter handler shows not supported message."""
        with patch('builtins.print') as mock_print:
    ...
    def test_broadcast_to_leaves_still_included(self):
        ...
        discovered_functions = self.handler.fp_functions
        self.assertIn('broadcast_to_leaves', discovered_functions, "broadcast_to_leaves should be included in CLI interface")
