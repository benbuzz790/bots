...
from bots.dev.cli import CLI, DynamicParameterCollector, DynamicFunctionalPromptHandler
...
def test_collect_skip_labels():
    """Test collecting skip labels (legacy functionality still used by parameter collector)."""
    collector = DynamicParameterCollector()
    with patch('builtins.input', return_value='draft, test, incomplete'):
        result = collector._collect_skip_labels('skip', None)
        assert result == ['draft', 'test', 'incomplete']
...
def test_collect_skip_labels_empty():
    """Test collecting empty skip labels (legacy functionality still used by parameter collector)."""
    collector = DynamicParameterCollector()
    with patch('builtins.input', return_value=''):
        result = collector._collect_skip_labels('skip', None)
        assert result == []
...
@patch('bots.dev.cli.create_tool_result_callback')
@patch('builtins.input')
@patch('builtins.print')
def test_broadcast_fp_new_system_all_leaves(mock_print, mock_input, mock_callback):
...
@patch('builtins.input')
@patch('builtins.print')
def test_broadcast_fp_new_system_specific_leaves(mock_print, mock_input, mock_callback):
    """Test broadcast_fp with new leaf selection system - select specific leaves."""
    # Setup mock bot and context
    mock_bot = Mock()
    mock_context = Mock()
    mock_conversation_handler = Mock()
