...
class TestRecombineCommandRemoval(unittest.TestCase):
    ...
    def test_combine_leaves_command_exists(self):
        ...
        with patch('builtins.input', side_effect=['Create a simple function', '/combine_leaves', '/exit']):
            with StringIO() as buf, redirect_stdout(buf):
                with self.assertRaises(SystemExit):
                    cli_module.main("")
                output = buf.getvalue()
        # Should not show "Unrecognized command" - instead should show proper error about leaves
