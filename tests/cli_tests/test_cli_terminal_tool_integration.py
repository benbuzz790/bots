...
class TestCLIRealTerminalTimeouts(DetailedTestCase):
    ...
    def _create_cli_with_timeout_monitoring(self):
        ...
        cli = cli_module.CLI()
        cli.context.bot_instance = AnthropicBot(allow_web_search=False)
    ...
    def test_powershell_here_string_real_bot(self, mock_input):
        ...
        mock_input.side_effect = [
            'Create a Python file called test_script.py using PowerShell here-strings. The file should contain a function that imports subprocess and json, with f-strings and encoding parameters like encoding="utf-8"',
            '/exit'
        ]
    ...
    def test_complex_python_file_real_bot(self, mock_input):
        ...
        mock_input.side_effect = [
            '''Create a Python file called complex_script.py using PowerShell. The file should contain:
1. A function with type hints and string defaults like: func(repo: str = "promptfoo/promptfoo")
2. F-strings with complex variables like: f"repos/{repo}/pulls/{pr_id}/reviews"
