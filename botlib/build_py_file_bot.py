...
This module provides a factory function and configuration for creating specialized
bot instances that write and test Python code. It encapsulates bot initialization,
tool configuration, and system message setup in a reusable way.
...
Note:
    This bot maintains state and saves it to disk. Each initialization will create
    a new state file at BOT_SAVE_PATH. The bot uses the Anthropic API and standard
    rate limits apply.
...
def create_file_writer_bot(autosave: bool = False) -> Bot:
    ...
    Use when you need to create a new bot instance for writing and testing Python files.
    This factory function handles all aspects of bot configuration including:
    ...
    Args:
        autosave (bool, optional): Whether to enable automatic state saving. Defaults to False.

    Returns:
        Bot: Fully configured file writer bot instance with all necessary tools and settings.
            The returned bot is ready to handle Python file creation and testing tasks.

    Raises:
        ValueError: If required environment variables for API access are not set
