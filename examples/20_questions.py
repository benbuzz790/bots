...
def main():
    ...
    def should_stop(bot):
            if bot.conversation.parent:
                return (
                    word in bot.conversation.parent.parent.content or
                    bot.conversation._node_count() > 50
                    )
            else:
                return False
    ...
    haiku_prompt = lambda word, question: f"You're a 20 questions judge. Answer              \
        with yes, no, sometimes, 'I don't know,' 'you got it!', or 'It was '{word}''.        \
        Do not say anything else. Do not explain your reasoning. You are the arbiter.        \
        Here is what you're judging: the thing is '{word}'. The question is '{question}'"
