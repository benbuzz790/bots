...
Use this module when you need to automatically review and improve code-level documentation
across a Python project using LLM-powered bots.
...
NOTE: This is an expensive operation. For a project this size, expect $20 - $40 in api costs
...
def find_python_files(start_path: str, exclude_list: List[str] = []) -> List[str]:
    """Recursively find all Python files in a directory with specific exclusions.

    Use when you need to gather Python files for documentation review while excluding
    tool-related files and specific directories that don't need documentation review.

    Parameters:
        start_path (str): Root directory to start search from. Should be the project root.
        exclude_list (List[str]): list of subdirectory names to exclude from search. (default: [])

    Returns:
        List[str]: List of absolute paths to Python files that should be reviewed
...
def create_doc_review_bot(num: int) -> Bot:
    ...
    bot.set_system_message("You are an expert in Python documentation standards and best practices. Your task is to review and improve code documentation using Python's standard documentation patterns including type hints, and docstrings. You do not change code or add features, you strictly document. When you find docs lacking, use the available tools to fix them directly.")
    return bot

def review_file_documentation(bot: Bot) -> Tuple[List[str], List[ConversationNode]]:
    """Execute a thorough documentation review process for a Python file.

    Use when you need to systematically review and improve documentation in a Python file
    using a chain of prompts and automated fixes.

    Parameters:
        bot (Bot): Configured documentation review bot that has been initialized with
    ...
    """
    prompts = ["First, let's read the project context to understand documentation standards. Please read bot_context.txt and README.md.", 
               "Now, let's examine the TARGET FILE. Use the view tool to read this file.", 
               textwrap.dedent('''
                    Review the file's documentation and make improvements where needed. For each issue you find, 
                    use the appropriate tools to fix it directly. Check for:
                    1. Module-level docstring:
    ...
    return fp.chain_while(bot, prompts, stop_condition=fp.conditions.tool_not_used_debug, continue_prompt='ok')
...
def main():
    ...
    for bot, file in zip(bots, python_files):
        bot.respond(f"NOTE: The TARGET FILE is {file}. Respond with exclusively 'ok' if you understand. Further instructions are incoming.")
