...
1. chain() - Sequential conversation where each prompt builds on previous responses
   Returns: Tuple[List[str], List[ConversationNode]] - responses and conversation nodes
...
The example uses a playful conversation to demonstrate how the same prompts can produce
different conversation patterns when used with chain vs branch approaches.
...
# Initialize bots for different conversation patterns (each maintains its own conversation tree)
...
# Each prompt builds on assumed previous responses to demonstrate context handling.
prompt_set: List[str] = [
...
# Demonstrate sequential conversation pattern (each response sees all previous context)
responses: List[str]
...
# Demonstrate parallel conversation branches (each response starts from root context)
responses: List[str]
nodes: List[ConversationNode]
