...
def clone_and_edit(local_dir: Path, repo_name: str, task_prompt: str, bot: Bot=None) -> Tuple[List[str], List[ConversationNode]]:
    """Clone a GitHub repository and allow a bot to work on it until completion.
    ...
    This function is a specialized version of clone_and_fp that uses the prompt_while
    pattern to enable iterative work on a repository until the bot stops using tools.
    ...
    Args:
        ...
        bot (Bot, optional): Bot instance to use. If None, creates a new AnthropicBot
            with code_tools and terminal_tools added
    ...
    Examples:
        ...
        ... )
    """
    first_prompt = f"I've cloned the repository {repo_name}. First, use view_dir with target_extensions=['*'] to see all files, then {task_prompt}"
...
def clone_and_fp(local_dir: Path,
                 repo_name: str,
    ...
    Use when you need to:
    - Clone a GitHub repository and immediately process it with a functional pattern
    - Apply complex reasoning patterns (chain, branch, tree_of_thought) to repo analysis
    - Maintain bot state for future operations on the same repository

    This function combines GitHub repository cloning with the flexibility of functional
    prompt patterns, allowing sophisticated analysis and modification of repository
    content using any of the patterns defined in functional_prompts.py.

    Args:
        local_dir (Path): Relative path to location where repo will be cloned
        repo_name (str): Name of repo to clone (format: "owner/repository")
        functional_prompt (fp.FunctionalPrompt): The functional pattern to apply
            (e.g., fp.chain, fp.branch, fp.tree_of_thought)
    ...
    Note:
        ...
        - The function uses gh cli for cloning - ensure it's installed and configured
    """
    ...
    if not clone_path.exists():
        result = execute_powershell(f'gh repo clone {repo_name} {str(clone_path)}')
    ...
    if not kwargs.get('first_prompt'):
        if isinstance(kwargs.get('prompts'), list):
            kwargs['prompts'][0] = f"I've cloned the repository {repo_name} to {str(clone_path)}. " + kwargs['prompts'][0]
