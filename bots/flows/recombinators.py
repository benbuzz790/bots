...
class recombinators:
    ...
    @staticmethod
    def llm_judge(responses: List[Response], nodes: List[ResponseNode], judge_bot: Optional[Bot]=None, instruction: str="Select the best response from the options below. Return only the selected response without additional commentary.", **kwargs) -> Tuple[Response, ResponseNode]:
        ...
        try:
            ...
            # Try to find which option was selected by looking for "Option X" in response
            import re
            ...
            return (f"Judge error: {str(e)}\nFallback to first response:\n{valid_responses[0][0]}", valid_responses[0][1])

    @staticmethod
    def llm_vote(responses: List[Response], nodes: List[ResponseNode], judge_bots: Optional[List[Bot]]=None, num_judges: int=3, instruction: str="Select the best response from the options below. Return only the number of your choice (1, 2, 3, etc.).", **kwargs) -> Tuple[Response, ResponseNode]:
    ...
    @staticmethod
    def llm_merge(responses: List[Response], nodes: List[ResponseNode], merger_bot: Optional[Bot]=None, instruction: str="Combine the following responses into a single, coherent, non-redundant response that captures the best insights from each:", **kwargs) -> Tuple[Response, ResponseNode]:
