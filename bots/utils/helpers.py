...
def _get_new_files(start_time: float, directory: str = ".", extension: str = None) -> List[str]:
    ...
    Performs a recursive search through the directory tree and its subdirectories.

    Parameters:
        start_time (float): Unix timestamp to filter files created after
        directory (str, optional): Root directory to start search from. Defaults to current directory
        extension (str, optional): File extension to filter by including the dot (e.g. '.py', '.txt'). 
            If None, returns files with any extension. Defaults to None

    Returns:
        List[str]: List of absolute file paths for files created after start_time.
            Paths are returned in the order they are discovered during directory traversal.
    """
    new_files = []
...
def _clean(code: str) -> str:
    """Clean and dedent code before parsing by removing common leading whitespace.
...
def _py_ast_to_source(node: ast.AST) -> str:
    ...
    Internal helper function for AST manipulation. Uses ast.unparse() to convert
    AST nodes back to source code. Works with any valid Python AST node type
    (Module, FunctionDef, ClassDef, etc.).

    Parameters:
        node (ast.AST): The AST node to convert. Must be a valid node from the ast module.
            Common types include: ast.Module, ast.FunctionDef, ast.ClassDef, ast.Expr

    Returns:
        str: The source code representation of the AST node. The output will be
            valid Python code that could be executed or compiled.

    Note:
        Requires Python 3.9+ for ast.unparse(). Comments and formatting from the
        original source code are not preserved in the output.
    """
    return ast.unparse(node)

def remove_code_blocks(text: str) -> Tuple[List[str], List[str]]:
    """Extract code blocks and their language labels from markdown-formatted text.

    Use when you need to parse markdown text containing fenced code blocks 
    (```language code```) and separate the code blocks from the rest of the text.
    Handles multiple code blocks and preserves their order. The function matches
    code blocks using a regex pattern that supports optional language labels and
    ...
    Returns:
        Tuple[List[str], List[str]]: A tuple containing:
            - List[str]: Extracted code blocks with whitespace trimmed, in order of appearance
