    ...
    try:
        ...
        for hunk in hunks:
            ...
            if adjusted_start <= len(current_lines):
                found, was_whitespace = check_match_type(current_lines, adjusted_start, context_before, removals)
                if found:
                    if was_whitespace:
            ...
            if not found:
                ...
                for i in range(1, len(current_lines) - len(context_before) + 2):
                    found, was_whitespace = check_match_type(current_lines, i, context_before, removals)
                    if found:
                        if was_whitespace:
                ...
                if matches:
                    ...
                    found = True
            if not found:
                _, best_line, match_quality, _ = find_block_in_content(current_lines, context_before, ignore_whitespace=True)
                if match_quality > 0.8:
                    ...
                    return f'Error: Context mismatch at line {old_start}\nExpected:\n{context_before}\nFound:\n{context}'
            pos = match_line - 1
            original_indent = ''
            if len(current_lines) > 0:
                if pos + len(context_before) < len(current_lines):
                    if removals:
                        original_indent = get_line_indentation(current_lines[pos + len(context_before)])
                    else:
                        original_indent = get_line_indentation(current_lines[pos + len(context_before) - 1])
                elif pos > 0:
                    original_indent = get_line_indentation(current_lines[pos - 1])
...
def find_block_in_content(content_lines: list, block_lines: list, ignore_whitespace: bool=False) -> tuple[bool, int, float, bool]:
...
def preserve_indentation(original_line: str, new_line: str) -> str:
    """Preserve indentation from original line when replacing with new line."""
    original_indent = get_line_indentation(original_line)
    ...
    try:
        ...
        for hunk in hunks:
            ...
            if not context_matches:
                found, match_line, match_quality = find_block_in_content(current_lines, context_before)
                if found:
...
def check_exact_match(content_lines: list, start_pos: int, context_lines: list) -> bool:
...
def check_whitespace_match(content_lines: list, start_pos: int, context_lines: list) -> bool:
...
def check_match_type(content_lines: list, start_pos: int, context_lines: list, removal_lines: list=None) -> tuple[bool, bool]:
    """
