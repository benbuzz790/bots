...
def load_code(code_or_filename: str) -> str:
    ...
    **SECURITY WARNING**: This function uses exec() to execute arbitrary Python code.
    ...
    It should only be used with trusted code sources. Executing untrusted code can:
    ...
    The exec() namespace is restricted to limit access to dangerous builtins,

    but determined attackers may still find ways to escape restrictions.
    ...
    warnings.warn(
        "load_code() is executing arbitrary code via exec(). " "This poses security risks if the code is untrusted.",
        RuntimeWarning,
        stacklevel=2,
    )
    ...
    try:
        ...
        # Allow only safe builtins and prevent dangerous operations
        ...
        restricted_globals = {
            ...
            "__name__": temp_module.__name__,
            "__doc__": temp_module.__doc__,
        }

        # Execute code in restricted namespace

        exec(code_or_filename, restricted_globals)
