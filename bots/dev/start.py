...
2. Project Analysis: Initializes project tree analysis using a specification file,
   which defines the structure and requirements for project analysis.
...
def cleanup_directory(script_path: str, extensions: Tuple[str, ...] = (".md", ".txt", ".bot", ".py"), dry_run: bool = True) -> List[str]:
    """Clean up a directory by removing files with specific extensions while preserving critical files.

    Use when you need to clean a project directory of generated files or temporary outputs
    while ensuring critical scripts are preserved. The function provides a safe way to
    delete files through its dry-run capability and preservation rules.

    Parameters:
    - script_path (str): Absolute or relative path to the script. Used to determine the
                        directory to clean and which script to preserve
    - extensions (Tuple[str, ...]): Tuple of file extensions to delete (including the dot).
                                   Default: (".md", ".txt", ".bot", ".py")
    - dry_run (bool): If True, only simulates deletion and prints affected files.
                     If False, performs actual deletion. Default: True

    Returns:
    List[str]: Names of files that were deleted (or would be deleted in dry-run mode).
              Empty list if no files match criteria or if errors occur during deletion.
    ...
    Example:
        ...
        >>> deleted = cleanup_directory("path/to/script.py", extensions=(".txt", ".bot"), dry_run=True)
...
def main():
    ...
    1. Cleanup: Removes specified file types after a dry run and user confirmation
    2. Analysis: If a spec file is provided as an argument, runs project tree analysis
