...
def make_bot_interruptible(bot: Bot) -> None:
    """Wrap a bot respond method to make it interruptible with Ctrl-C.
    This allows users to interrupt long-running bot operations (like API calls)
    by pressing Ctrl-C. The bot state remains consistent after interruption.
    Args:
        bot: The bot instance to make interruptible
    Note:
        This modifies the bot respond method in-place by wrapping it with
