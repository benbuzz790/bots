...
class OpenAIToolHandler(ToolHandler):
    ...
    def generate_tool_schema(self, func: Callable) ->Dict[str, Any]:
        ...
        schema = {'type': 'function', 'function': {'name': func.__name__,
        ...
        for param_name, param in sig.parameters.items():
            schema['function']['parameters']['properties'][param_name] = {
                'type': 'string', 'description': f'Parameter: {param_name}'}
    ...
    def tool_name_and_input(self, request_schema: Dict[str, Any]) ->Tuple[
        ...
        request = {'id': tool_call['id'], 'type': tool_call['type'], 'name':
            tool_call['function']['name'], 'arguments': json.loads(
    ...
    def generate_error_schema(self, error_msg: str, request_schema: Optional[
        ...
        """
        return {'role': 'tool', 'content': error_msg, 'tool_call_id': 
            request_schema['tool_calls'][0]['id'] if request_schema and 
            'tool_calls' in request_schema else 'unknown'}
...
class OpenAIMailbox(Mailbox):
    ...
    def send_message(self, bot) ->Dict[str, Any]:
        ...
        if tools:
            response = self.client.chat.completions.create(model=model,
                messages=messages, max_tokens=max_tokens, temperature=
                temperature, tools=tools, tool_choice='auto')
        else:
            response = self.client.chat.completions.create(model=model,
                messages=messages, max_tokens=max_tokens, temperature=
                temperature)
    ...
    def process_response(self, response: Dict[str, Any], bot: Bot) ->Tuple[
...
class ChatGPT_Bot(Bot):
    ...
    def __init__(self, api_key: Optional[str]=None, model_engine: Engines=
        Engines.GPT4, max_tokens: int=4096, temperature: float=0.3, name:
        str='bot', role: str='assistant', role_description: str=
        'a friendly AI assistant', autosave: bool=True):
