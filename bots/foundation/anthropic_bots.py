...
The main class is AnthropicBot, which provides a complete implementation ready for
...
Classes:
    AnthropicNode: Conversation node implementation for Anthropic's message format
    ...
    CacheController: Manages conversation history caching for context optimization
"""
...
class AnthropicNode(ConversationNode):
    ...
    This class extends ConversationNode to handle Anthropic-specific message formatting,
    including proper handling of tool calls and results in the conversation tree.
    ...
    def _add_tool_results(self, results: List[Dict[str, Any]]) -> None:
        ...
        For Anthropic bots, tool results need to be propagated to the replies or stored as
    ...
    def _build_messages(self) -> List[Dict[str, Any]]:
        ...
        Constructs the message history in Anthropic's expected format, properly handling
...
class AnthropicToolHandler(ToolHandler):
    ...
    This class manages the conversion between Python functions and Anthropic's tool format,
    handling schema generation, request/response formatting, and error handling.
    """
    ...
    def generate_tool_schema(self, func: Callable) -> Dict[str, Any]:
        ...
        tool: Dict[str, Any] = {'name': func.__name__, 'description': doc, 'input_schema': {'type': 'object', 'properties': {}, 'required': []}}
    ...
    def generate_response_schema(self, request: Dict[str, Any], tool_output_kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        response: Dict[str, Any] = {'type': 'tool_result', 'tool_use_id': request.get('id', 'unknown id'), 'content': tool_output_kwargs}
        return response
    ...
    def generate_error_schema(self, request_schema: Optional[Dict[str, Any]], error_msg: str) -> Dict[str, Any]:
        ...
        """
        return {'type': 'tool_result', 'tool_use_id': request_schema.get('id', 'unknown id') if request_schema else 'unknown id', 'content': error_msg}

class AnthropicMailbox(Mailbox):
    """Handles communication with Anthropic's API.

    This class manages message sending and response processing for Anthropic models,
    including retry logic for API errors and handling of incomplete responses.
    ...
    def send_message(self, bot: 'AnthropicBot') -> Dict[str, Any]:
        ...
        Handles API key setup, message formatting, and implements exponential backoff
        ...
        non_optional = {'model': model, 'max_tokens': max_tokens, 'temperature': temperature, 'messages': messages}
...
class AnthropicBot(Bot):
    ...
    Use when you need to create a bot that interfaces with Anthropic's chat completion models.
    ...
    Attributes:
        ...
        role_description (str): Detailed description of bot's role/personality (for humans to read, not used in api)
...
class CacheController:
    ...
    This class handles the placement and management of cache control markers in the
    conversation history to optimize context window usage. It ensures that cache
    control directives are properly placed and don't interfere with tool operations.

    The controller maintains a balance between preserving important context and
    allowing older messages to be cached or dropped when the context window fills up.
    """
    ...
    def shift_cache_control_out_of_tool_block(self, messages: List[Dict[str, Any]], position: int) -> int:
        ...
        Cache controls should not be placed within tool call or result blocks as this
        ...
        if has_tool_block:
            ...
            while new_position < len(messages):
                ...
                if isinstance(next_content, list):
                    is_tool = any(isinstance(item, dict) and item.get('type') in ['tool_call', 'tool_result']
    ...
    def manage_cache_controls(self, messages: List[Dict[str, Any]], threshold: float = 5.0) -> List[Dict[str, Any]]:
        ...
        Use when you need to optimize the conversation context window by managing which
