...
class DevelopmentRunner:
    ...
    def start_frontend(self) -> Optional[subprocess.Popen]:
        ...
        try:
            ...
            process = subprocess.Popen([
                "npm", "run", "dev"
            ], cwd=self.frontend_dir, shell=True)  # Add shell=True for Windows compatibility
    ...
    def wait_for_backend(self, timeout: int = 120) -> bool:  # Increased timeout to 120 seconds
        ...
        while time.time() - start_time < timeout:
            try:
                response = requests.get("http://127.0.0.1:8000/api/health", timeout=10)  # Increased request timeout
                if response.status_code == 200:
                    logger.info("Backend is ready!")
                    logger.info(f"Backend health response: {response.json()}")
                    return True
                else:
    ...
    def wait_for_frontend(self, timeout: int = 120) -> bool:  # Increased timeout
        """Wait for frontend to be ready."""
        logger.info("Waiting for frontend to be ready...")

        # Try multiple ports since Vite might use a different port if 3000 is busy
        ...
        while time.time() - start_time < timeout:
            for port in ports_to_try:
                try:
                    ...
                    if response.status_code == 200:
                        logger.info(f"Frontend is ready on port {port}!")
                        return True
