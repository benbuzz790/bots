...
def main():
    ...
    test_configs = [
        {
            "args": ["--timeout=30", "-v", "--disable-warnings", "--tb=short", "test_docker_permissions.py"],
            "description": "Docker permissions test (safe baseline)",
            "timeout": 60,
            "critical": True  # Must pass for others to be meaningful
        },
        {
            "args": ["--timeout=30", "-v", "--disable-warnings", "--tb=short", "-k", "test_web_tool and not integration"],
            "description": "Web tool tests (basic functionality)",
            "timeout": 120,
            "critical": False
        },
        {
            "args": ["--timeout=30", "-v", "--disable-warnings", "--tb=short", "-k", "test_toolify and not integration"],
            "description": "Toolify tests (core functionality)", 
            "timeout": 120,
            "critical": False
        },
        {
            "args": ["--timeout=30", "-v", "--disable-warnings", "--tb=short", "tests/test_cli/test_broadcast_fp_comprehensive.py", "-x"],
            "description": "Broadcast FP tests (investigating failures)",
            "timeout": 180,
            "critical": False
        },
        {
            "args": ["--timeout=30", "-v", "--disable-warnings", "--tb=short", "-k", "not (test_branch_self_recursive or test_real_ai_stash_message_generation)", "--maxfail=5"],
