...
import os
import inspect
from cli2 import DynamicParameterCollector, DynamicFunctionalPromptHandler, CLI2
...
def test_cli2_initialization():
    """Test that CLI2 can be initialized."""
    print("\nTesting CLI2 initialization...")
    try:
        cli = CLI2()
        print(f"CLI2 created with {len(cli.commands)} commands:")
        for cmd in sorted(cli.commands.keys()):
            print(f"  {cmd}")
        # Test that handlers are properly initialized
        if not hasattr(cli, 'context'):
            print("ERROR: CLI2 missing context")
            return False
        if not hasattr(cli, 'fp'):
            print("ERROR: CLI2 missing fp handler")
            return False
        if cli.context is None:
            print("ERROR: CLI2 context is None")
            return False
        if cli.fp is None:
            print("ERROR: CLI2 fp handler is None")
            return False
        print("All handlers initialized correctly")
        return True
    except Exception as e:
        print(f"Error initializing CLI2: {e}")
        import traceback
        traceback.print_exc()
        return False
if __name__ == "__main__":
    ...
    try:
        ...
        success &= test_cli2_initialization()
        if success:
