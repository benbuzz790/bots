...
- **Lazy Decorator**: Utilize the `@lazy()` decorator from `lazy.py` to tag functions as LLM responsibilities. These functions are lazily generated and executed the first time they are called, with customizable context levels. [Demo](https://drive.google.com/file/d/1UMDceuhx4KPnoQZblyXqNvfMiMO5aXXq/view?usp=drivesdk)
...
### 6. Lazy Decorator
`lazy.py` defines a Python function decorator `@lazy()` that leverages LLMs to dynamically generate function implementations. Key features include:
...
def complex_algorithm(input_data):
    ...
    The implementation will be generated by an LLM when first called.
    """
    pass
```

This approach allows for dynamic code generation tailored to the specific context and requirements of your project.
